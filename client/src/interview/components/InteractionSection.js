...
function InteractionSection({ question, loading, setCurrentQuestion, setSessionId }) {
    ...
    try {
      ...
      const res = await fetch("/api/interview/start", {
        ...
        body: JSON.stringify({
          ...
          jobDescription: "React, JavaScript, APIs",
          resumeText: "Mock resume text for MVP",
        }),
      });

      const data = await res.json();
      console.log("AI FIRST RESPONSE:", data);

      // Set the question and session
      if (data.ai && data.ai.type === "question") {
        setCurrentQuestion({
          text: data.ai.content,
          type: "text" // assuming text questions for now
        });
      }

      /* 2️⃣ MICROPHONE + AUDIO ANALYSER */
